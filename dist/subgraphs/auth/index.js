...
await mongoose.connect(process.env.MONGO_URI || "mongodb://localhost:27017/water_auth");
console.log("BOOT (AUTH) USER_SUBGRAPH_URL =", process.env.USER_SUBGRAPH_URL);
// ⭐⭐⭐ 核心：创建 DI 容器实例（一次）
...
app.use("/graphql", cors({ origin: "http://localhost:3000", credentials: true }), express.json(), cookieParser(), expressMiddleware(server, {
