...
export default class LoginRiskService {
    ...
    async handleRefreshTokenReuse({ userId, ip, userAgent, refreshTokenId, }) {
    ...
    async evaluate({ userId, ip, deviceId, }) {
        let risk = 0;
        const flags = [];
        ...
        // 3️⃣ 新设备
        const isKnownDevice = await redis.sismember(`user:${userId}:knownDevices`, deviceId);
        if (!isKnownDevice) {
            risk += 30;
    ...
    async recordSuccess({ userId, ip, deviceId, country, }) {
        ...
        pipeline.sadd(`user:${userId}:knownDevices`, deviceId);
        await pipeline.exec();
    }
    ...
    async freezeUser(userId) {
        await redis.set(`user:${userId}:security`, JSON.stringify({
            status: "FROZEN",
