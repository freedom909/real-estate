...
import { FIND_USER_BY_ID, FIND_USER_BY_EMAIL, CREATE_OAUTH_USER, } from "./queries.js";
export default class UserClient {
    ...
    async findById(id) {
        if (!id)
            return null;
        const res = await this.client.request(FIND_USER_BY_ID, { id });
        return res?.userById ?? null;
    }
    async findByEmail(email) {
        if (!email)
            return null;
        const res = await this.client.request(FIND_USER_BY_EMAIL, { email });
    ...
    async createOAuthUser(input) {
        ...
        const res = await this.client.request(CREATE_OAUTH_USER, { input });
