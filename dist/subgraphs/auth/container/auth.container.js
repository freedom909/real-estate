...
export function createAuthContainer({ redis, userApi, refreshTokenRepo }) {
    ...
    container.register(TOKENS.auth.userRepo, () => new UserRepo({
    ...
    // ======================================================
    container.register(TOKENS.auth.userGraphQLClient, () => createUserGraphQLClient());
    // ======================================================
    // ACL — UserClient (Auth → User Subgraph)
    // ======================================================
    container.register(TOKENS.auth.userClient, () => new UserClient(container.resolve(TOKENS.auth.userGraphQLClient)));
    // ======================================================
    // MODELS
    // ======================================================
    container.register(TOKENS.auth.credentialModel, () => CredentialModel);
    container.register(TOKENS.auth.refreshTokenModel, () => RefreshTokenModel);
    // ======================================================
    // REPOSITORIES
    // ======================================================
    container.register(TOKENS.auth.credentialRepo, () => new CredentialRepo({
        CredentialModel: container.resolve(TOKENS.auth.credentialModel),
    }));
    container.register(TOKENS.auth.refreshTokenRepo, () => new RefreshTokenRepo({
        RefreshTokenModel: container.resolve(TOKENS.auth.refreshTokenModel),
    }));
    container.register(TOKENS.infra.cache, () => new RedisAdapter(redis));
    container.register(TOKENS.auth.riskEventRepo, () => new RiskEventRepo({ redis: container.resolve(TOKENS.infra.cache) }));
    // ======================================================
    // DOMAIN SERVICES
    // ======================================================
    container.register(TOKENS.auth.loginRiskService, () => new LoginRiskService({
        riskEventRepo: container.resolve(TOKENS.auth.riskEventRepo),
    }));
    container.register(TOKENS.auth.tokenService, () => new TokenService());
    container.register(TOKENS.auth.refreshTokenService, () => new RefreshTokenService({
    ...
    container.register(TOKENS.auth.oauthAdapter, () => new OAuthAdapter({
        google: new GoogleOAuthAdapter({
            clientId: process.env.GOOGLE_CLIENT_ID,
        }),
    }));
    container.register(TOKENS.auth.oauthVerifier, () => new OAuthVerifier());
    container.register(TOKENS.auth.oauthAccountRepo, () => new OAuthAccountRepo({ model: OAuthAccountModel }));
    // ======================================================
    // APPLICATION SERVICE
    // ======================================================
    container.register(TOKENS.auth.oauthAccountModel, () => OAuthAccountModel);
    container.register(TOKENS.auth.mergeAccountService, () => new MergeAccountService({
        ...container.resolve(TOKENS.auth.mergeAccountService),
        userRepo: container.resolve(TOKENS.auth.userRepo),
    }));
    container.register(TOKENS.auth.authService, () => new AuthService({
        oauthService: container.resolve(TOKENS.auth.oauthAdapter),
        oauthAccountRepo: container.resolve(TOKENS.auth.oauthAccountRepo),
        userClient: container.resolve(TOKENS.auth.userClient),
        tokenService: container.resolve(TOKENS.auth.tokenService),
        refreshTokenRepo: container.resolve(TOKENS.auth.refreshTokenService),
        loginRiskService: container.resolve(TOKENS.auth.loginRiskService),
        credentialRepo: container.resolve(TOKENS.auth.credentialRepo),
        sessionRepo: container.resolve(TOKENS.auth.refreshTokenRepo),
    }));
    return container;
}
