...
export const buildAuthLoginEvent = ({ user, req, failedLoginCount }) => SecurityEventBuilder.create('AUTH_LOGIN')
    .withActor({
    userId: user.userId,
    role: user.role,
})
...
export const buildApiRequestEvent = ({ req, user }) => SecurityEventBuilder.create('API_REQUEST')
    ...
    .build();
export const buildTokenUsageEvent = ({ token, req }) => SecurityEventBuilder.create('TOKEN_USAGE')
    .withActor({
    userId: token.sub,
    role: token.role,
})
